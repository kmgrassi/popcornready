...
export const POPCORN_READY_SCREEN_REFERENCE = path.join(
  process.cwd(),
  "public",
  "reference",
...
export function shouldUsePopcornReadyScreenReference(goal: string): boolean {
  return (
    /\bpopcorn\s+ready\b/i.test(goal) &&
    /\b(screen|website|web site|landing page|computer|laptop)\b/i.test(goal)
  );
}
