...
// Cancels an active run when the backend supports it. PR 4 returns a 501
// "not_implemented" envelope until the backend wires cancellation in PR 3.
...
export async function POST(
  ...
  try {
    ...
    throw new ApiError(
      "not_implemented",
      "Cancel is not supported for generation runs yet.",
      { supported: false, action: "cancel" }
    );
  } catch (err) {
