...
Expose Aividi video creation through stable `/api/v1` endpoints designed for
...
- Prompt-only requests where Aividi generates the required visuals, narration,
  timeline, and export without pre-supplied media.
- Hybrid requests where agents provide some assets and ask Aividi to fill gaps.
...
brand constraints. Aividi creates missing media assets first, then builds the
timeline and export.
...
The agent supplies some source assets and asks Aividi to fill missing shots,
...
If `providerPolicy` is omitted, Aividi uses configured defaults. Agents can
