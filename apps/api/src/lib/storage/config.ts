...
export function readStorageConfig(
  ...
  const config: StorageConfig = {
    ...
    presignTtlSeconds: positiveInteger(env.S3_PRESIGN_TTL_SECONDS, 900),
    multipartThresholdBytes: positiveInteger(
      env.S3_MULTIPART_THRESHOLD_BYTES,
      100 * 1024 * 1024
    ),
    multipartPartSizeBytes: Math.max(
      5 * 1024 * 1024,
      positiveInteger(env.S3_MULTIPART_PART_SIZE_BYTES, 64 * 1024 * 1024)
    ),
  };
