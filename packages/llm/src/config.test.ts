...
test("hasLlmCredentials checks the configured provider's key", async () => {
  const { hasLlmCredentials } = await import("./config");
  assert.equal(hasLlmCredentials({}), false);
  assert.equal(hasLlmCredentials({ OPENAI_API_KEY: "sk-x" }), true);
  // Anthropic key alone does not satisfy the default (openai) provider.
  assert.equal(hasLlmCredentials({ ANTHROPIC_API_KEY: "sk-a" }), false);
  assert.equal(
    hasLlmCredentials({ LLM_PROVIDER: "anthropic", ANTHROPIC_API_KEY: "sk-a" }),
    true
  );
  assert.equal(
    hasLlmCredentials({ LLM_PROVIDER: "anthropic", ANTHROPIC_API_KEY: "  " }),
    false
  );
});
