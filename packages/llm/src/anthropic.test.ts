...
test("modelFor reports the same routing structured/chooseTool use", () => {
  ...
  assert.equal(client.modelFor("minimal"), "claude-haiku");
  assert.equal(client.modelFor("low"), "claude-haiku");
  assert.equal(client.modelFor("medium"), "claude-x");
  assert.equal(client.modelFor("high"), "claude-x");
  assert.equal(client.modelFor(), "claude-x");
});
